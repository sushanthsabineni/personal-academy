...
export default function CreditsPageBackup() {
  ...
  // Usage stats placeholder (replace with real data if available)
  const usageStats = [
    { label: 'Storyboards', value: creditsData?.stats.storyboardsCreated ?? 0, sublabel: 'Created' },
    { label: 'Earned', value: creditsData?.stats.totalEarned ?? 0, sublabel: 'Credits' },
    { label: 'Spent', value: creditsData?.stats.totalSpent ?? 0, sublabel: 'Credits' },
    { label: 'Purchased', value: creditsData?.stats.totalPurchased ?? 0, sublabel: 'Credits' },
  ];
  ...
  const filteredTransactions = creditsData?.transactions.filter((transaction) => {
    const matchesType =
      ...
      (filterType === 'purchased' && transaction.type === 'purchase') ||
  ...
  const generateInvoicePDF = async (transaction: Transaction) => {
    ...
    yPos += 4
    doc.text('support@personalacademy.app', 105, yPos, { align: 'center' })
    ...
    doc.setFont('helvetica', 'bold')
  doc.text(transaction.invoice_number || 'N/A', 165, yPos)
    
    yPos += 7
    doc.setFont('helvetica', 'normal')
    doc.text('Date:', 130, yPos)
    doc.setFont('helvetica', 'bold')
  doc.text(new Date(transaction.created_at).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    }), 165, yPos)
  ...
  doc.text(`$${transaction.payment_amount?.toFixed(2)}`, 185, yPos + 8, { align: 'right' })
        ...
        <div className="bg-gradient-to-br from-brand-teal to-brand-cyan rounded-2xl p-8 mb-8 text-white shadow-xl">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <h2 className="text-5xl font-bold mb-2">{(creditsData?.currentBalance ?? 0).toLocaleString()}</h2>
              <p className="text-white/90 text-sm">
                ≈ {Math.floor((creditsData?.currentBalance ?? 0) / 250)} storyboards remaining
              </p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-2xl font-bold text-light-text dark:text-dark-text">
                  {(creditsData?.stats.totalEarned ?? 0).toLocaleString()}
                </div>
          ...
          <div className="bg-light-card dark:bg-dark-card rounded-xl p-6 border border-light-border dark:border-dark-border">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-2xl font-bold text-light-text dark:text-dark-text">
                  {(creditsData?.stats.totalPurchased ?? 0).toLocaleString()}
                </div>
          ...
          <div className="bg-light-card dark:bg-dark-card rounded-xl p-6 border border-light-border dark:border-dark-border">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-2xl font-bold text-light-text dark:text-dark-text">
                  {(creditsData?.stats.totalSpent ?? 0).toLocaleString()}
                </div>
              ...
              filteredTransactions.map((transaction) => (
                ...
                >
                  ...
                  <div className={`text-right flex items-center gap-3`}>
                    ...
                    {transaction.type === 'purchase' && transaction.invoice_number && (
                      <button
          ...
          {filteredTransactions.length > 0 && (
            <div className="mt-6 pt-6 border-t border-light-border dark:border-dark-border text-center">
              <p className="text-sm text-light-muted dark:text-dark-muted">
                Showing {filteredTransactions.length} of {creditsData?.transactions.length ?? 0} transactions
              </p>
            </div>
          )}
        </div>
        ...
        {selectedInvoice && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              {(() => {
                const invoice = creditsData?.transactions.find(t => t.id === selectedInvoice)
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="text-right">
                          <div className="mb-4">
                            ...
                            <p className="font-bold text-lg text-light-text dark:text-dark-text">{invoice.invoice_number}</p>
                          </div>
                          <div className="mb-4">
